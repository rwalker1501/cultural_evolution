#!/usr/bin/env python
# -*- coding: utf-8 -*-  
import os
import main_module as mm


#mm.run_experiment("No equatorials HR.txt")
#mm.run_experiment("No equatorials exact direct_HR.txt")
mm.run_experiment("Timmermann no equatorials_HR(2).txt")
#mm.run_experiment("Timmermann no equatorials exact direct_HR.txt")
#mm.run_experiment("france_spain_HR.txt")
#mm.run_experiment("Australia_HR.txt")
#mm.run_experiment("No equatorials gt 10k_HR.txt")
#mm.run_experiment("No equatorials lt 10k_HR.txt")